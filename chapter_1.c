...
uint key_tab[] = {0x3f, 0x06, 0x5b, 0x4f, 0x66, 0x6d, 0x7d, 0x07, 0x7f, 0x6f, 0x77, 0x7c, 0x39, 0x5e, 0x79, 0x71, 0x80};
// delay 10ms
...
void display_key(uchar key, uint display_bus,uint display_bit_bus,uchar bit)
{
    ...
    uint xdata *display_bus_pointer;
...
void main()
{
    uint xdata write_bus = cs_key | 0x02; 
    uint xdata read_bus = cs_key | 0x01;
    uint xdata display_bus = cs_key | 0x04;
    uint xdata display_bit_bus = cs_key | 0x02;
    while (1)
    {
        /* code */
